...
class Joins(commands.Cog):
    ...
    async def cog_load(self):
        """This runs when the cog is loaded during bot startup."""
        try:
    ...
    async def get_guild_config(self, guild_id: int) -> dict:
        """Fetches the config from Mongo directly."""
    ...
    @commands.hybrid_command(name="joinsconfig", description="Configure server welcome, greet messages, and autoroles", with_app_command=True)
    @commands.has_permissions(manage_guild=True)
    async def joinsconfig(self, ctx):
        config = await self.get_guild_config(ctx.guild.id)
        embed = get_welcome_embed(config)
        view = WelcomeConfigView(self.collection, config)
